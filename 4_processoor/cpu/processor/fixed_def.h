...
OP2(mul, {
    ...
    if (sign1 == sign2) {
        if (/*inp_2 > (expand_type)(INT32_MAX / inp_1)*/(expand_type)(inp_1*inp_2)>= INT32_MAX) { //?
            return INT32_MAX; // inp_2 = inp_1 / INT32_MAX
        }
    }
    else if (/*abs(inp_2) > abs((expand_type)(INT32_MIN / inp_1))*/(expand_type)(inp_1*inp_2)<INT32_MIN) { //?
...
OP2(div, {
    ...
    else if (/*abs(inp_2) > abs((expand_type)(INT32_MAX * inp_1))*/(expand_type)inp_1/inp_2 < INT32_MIN) {
        return INT32_MIN;
    }
